...
export const maxDuration = 120; // Allow up to 120s for RSS + enrichment + AI generation

const REPORT_TIMEOUT_MS = 115 * 1000; // 115s — must finish before Vercel's 120s hard kill
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (message === 'REPORT_TIMEOUT') {
      errorType = 'TIMEOUT';
      statusCode = 504;
      userMessage = 'Report generation timed out after 2 minutes. The AI provider may be slow — please try again.';
    } else if (
