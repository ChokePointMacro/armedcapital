...
import { storeToken, getTokenStatus, initTokens } from '@/lib/tokenManager';

export const dynamic = 'force-dynamic';

/**
 * GET /api/tradingview/session — Check TV session status
 * POST /api/tradingview/session — Update TV session ID at runtime (persists to Supabase)
 */
...
export async function GET() {
  try {
    ...
    await initTokens();
    const { getConnectionStatus, isSessionValid } = await import('@/lib/tradingviewWS');
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Persist to Supabase so it auto-loads on next deploy/restart
    await storeToken('tradingview', 'session_id', cleaned);
    ...
    return NextResponse.json({
      success: true,
      message: 'Session ID updated and persisted. WebSocket reconnecting with new credentials.',
      authenticated: status.authenticated,
      connected: status.connected,
    });
  } catch (err) {
