...
export async function GET(request: NextRequest) {
  try {
    // CRON_SECRET is mandatory â reject all requests if unset
    const cronSecret = process.env.CRON_SECRET;
    if (!cronSecret) {
      console.error('[Cron] CRON_SECRET env var is not set â rejecting request');
    ...
    try {
      // Process scheduled social posts â actually post to X
      try {
      ...
      for (const post of postable) {
        try {
          ...
          // Post via shared xClient â prefer OAuth 2.0 DB tokens, fallback to OAuth 1.0a env vars
          let xResult;
          ...
          if (userToken?.access_token && userToken.access_token !== 'oauth1a-env') {
            ...
            await updateScheduledPostStatus(post.id, 'failed', {
              ...
              message: 'No X credentials available â connect your account in Settings',
            });
          ...
          if (xResult.success) {
            console.log(`[Cron] â Posted tweet ${xResult.tweetId} for scheduled post ${post.id}`);
            await updateScheduledPostStatus(post.id, 'posted');
            results.postsProcessed++;
          } else {
            ...
            if (xResult.code === 'RATE_LIMITED') {
              console.warn('[Cron] Rate limited â stopping batch');
              break;
            }
          }
    ...
    try {
      ...
      for (const schedule of schedules) {
        ...
        try {
          ...
          console.log(`[Cron] â Scheduled report generated: ${reportId}`);
          ...
          try {
            ...
            if (
              ...
              const html = `
                <div style="background:#0a0a0a;color:#e5e5e5;font-family:sans-serif;max-width:700px;margin:0 auto;padding:32px;">
                  ...
                  <div style="margin-top:32px;padding-top:16px;border-top:1px solid rgba(247,147,26,0.1);text-align:center;">
                    <p style="color:#444;font-family:monospace;font-size:10px;text-transform:uppercase;margin:0;">Intelligence Brief Â· Automated Digest</p>
                  </div>
                </div>`;
              ...
              await transporter.sendMail({
                ...
                to: digestEmail,
                subject: `${typeLabel} Brief â ${new Date().toLocaleDateString('en-US', {
                  month: 'long',
                  day: 'numeric',
                  year: 'numeric',
                })}`,
              ...
              results.emailsSent++;
              console.log(`[Cron] â Email digest sent to ${digestEmail}`);
            }
          } catch (emailErr) {
