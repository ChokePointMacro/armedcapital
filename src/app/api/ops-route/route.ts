...
export async function POST(req: NextRequest) {
  ...
  if (cmd) {
    try {
      ...
      return NextResponse.json({ ok: false, error: (err as Error).message, output: (err as any).stdout || '' }, { status: 500 });
    }
  }
...
export async function GET() { return NextResponse.json({status:'exec-ready'}); }
