...
const MODE_STYLES: Record<string, string> = {
  paper: 'border-yellow-400/50 bg-yellow-400/10 text-yellow-400',
  live: 'border-red-500/50 bg-red-500/10 text-red-500',
};
...
function RecentSignals({ signals }: { signals: WebhookSignal[] }) {
  if (signals.length === 0) {
    return (
      <div className="border border-gray-800/50 rounded-lg p-4 bg-gray-950/30">
        <div className="text-[10px] font-mono text-gray-600 uppercase tracking-wider mb-2">Recent Signals</div>
        <p className="text-[11px] font-mono text-gray-600">
          No webhook signals received yet. Configure TradingView alerts to send webhooks to /api/webhooks/tradingview
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="border border-gray-800 rounded-lg p-4 bg-gray-900/60">
      <div className="text-[10px] font-mono text-gray-600 uppercase tracking-wider mb-3">
        ...
        <table className="w-full text-[11px] font-mono">
          <thead>
            <tr className="border-b border-gray-800/50">
              <th className="text-left py-1.5 px-2 text-gray-600">Time</th>
              <th className="text-left py-1.5 px-2 text-gray-600">Ticker</th>
              <th className="text-left py-1.5 px-2 text-gray-600">Action</th>
              <th className="text-left py-1.5 px-2 text-gray-600">Price</th>
              <th className="text-left py-1.5 px-2 text-gray-600">Strategy</th>
              <th className="text-left py-1.5 px-2 text-gray-600">Source</th>
            </tr>
          </thead>
          <tbody>
            {signals.slice(0, 10).map((signal) => {
              const isAction = signal.action?.toUpperCase() || '';
              const isBuy = isAction === 'BUY' || isAction === 'LONG';
              const isSell = isAction === 'SELL' || isAction === 'SHORT';
              const actionColor = isBuy ? 'text-green-400' : isSell ? 'text-red-400' : 'text-gray-400';
              const signalTime = new Date(signal.timestamp).toLocaleTimeString('en-US', {
                hour: 'numeric',
                minute: '2-digit',
                second: '2-digit',
              });
              ...
              return (
                <tr key={signal.id} className="border-b border-gray-800/30 hover:bg-gray-800/20">
                  <td className="py-1.5 px-2 text-gray-500">{signalTime}</td>
                  <td className="py-1.5 px-2 text-gray-200 font-semibold">{signal.ticker}</td>
                  <td className={`py-1.5 px-2 ${actionColor}`}>{isAction}</td>
                  <td className="py-1.5 px-2 text-gray-400">
                    ${typeof signal.price === 'number' ? signal.price.toFixed(2) : signal.price}
                  </td>
                  <td className="py-1.5 px-2 text-gray-500">{signal.strategy || '—'}</td>
                  <td className="py-1.5 px-2 text-gray-600 text-[10px]">{signal.source}</td>
                </tr>
              );
            })}
          </tbody>
...
function ConfigPanel({
  ...
  const handleLiveClick = () => {
    if (config.trading_mode === 'paper') {
      ...
      onUpdate({ trading_mode: 'paper' });
    }
  };

  const confirmLive = () => {
    setShowLiveWarning(false);
    onUpdate({ trading_mode: 'live' });
  };
    ...
    <div className="space-y-4">
      ...
      {showLiveWarning && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-red-500/30 rounded-lg p-6 max-w-md">
            <div className="flex items-start gap-3 mb-4">
              <AlertTriangle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
              <div>
                <h3 className="font-mono text-sm text-red-500 font-bold">LIVE TRADING WARNING</h3>
                <p className="text-[10px] font-mono text-gray-400 mt-2">
                  Live mode requires verified broker credentials (exchange API keys configured on the server). Trades will use REAL funds and execute on live markets.
                </p>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 px-3 rounded border border-gray-800 bg-gray-950 text-gray-400 text-xs font-mono hover:border-gray-700 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={confirmLive}
                className="flex-1 py-2 px-3 rounded border border-red-500/30 bg-red-950/20 text-red-400 text-xs font-mono hover:bg-red-900/30 transition-colors"
              >
                Enable Live
              </button>
            </div>
...
export function TradingBot({ user }: { user: any }) {
  ...
  const loadAll = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const [posData, perfData, configData, logsData, signalsData] = await Promise.all([
        ...
        safeFetch('/api/webhooks/tradingview?limit=20'),
      ]);
      ...
      if (signalsData && Array.isArray(signalsData)) setSignals(signalsData);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black text-gray-200 p-4 md:p-6 max-w-6xl mx-auto">
      ...
      {botConfig?.trading_mode === 'paper' && (
        <div className="mb-4 p-3 rounded border border-yellow-400/30 bg-yellow-400/5 flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 text-yellow-400 flex-shrink-0" />
          <span className="text-xs font-mono text-yellow-400">PAPER MODE ONLY — Trades are simulated, no real funds at risk</span>
        </div>
      )}
          ...
          <h1 className="text-lg font-mono text-gray-200 tracking-tight flex items-center gap-2">
            ...
            {botConfig && (
              <span className={`text-[9px] font-mono px-2 py-0.5 rounded border ml-2 ${MODE_STYLES[botConfig.trading_mode]}`}>
                {botConfig.trading_mode === 'paper' ? '📄 PAPER' : '🚨 LIVE'}
              </span>
            )}
          </h1>
