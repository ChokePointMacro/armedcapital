...
export function checkBudget(agentId: string, category: string, estimatedCostUsd: number): { allowed: boolean; reason?: string } {
  const budget = getAgentBudget(agentId, category);

  if (budget.paused) {
    return { allowed: false, reason: `Agent ${agentId} is paused due to budget cap` };
  }

  if (budget.dailySpentUsd + estimatedCostUsd > budget.dailyLimitUsd) {
    return { allowed: false, reason: `Daily budget exceeded: $${budget.dailySpentUsd.toFixed(2)}/$${budget.dailyLimitUsd.toFixed(2)}` };
  }

  if (budget.monthlySpentUsd + estimatedCostUsd > budget.monthlyLimitUsd) {
    return { allowed: false, reason: `Monthly budget exceeded: $${budget.monthlySpentUsd.toFixed(2)}/$${budget.monthlyLimitUsd.toFixed(2)}` };
  }
...
export interface AuditEvent {
  id: string;
  type: 'task_execution' | 'pipeline_run' | 'budget_event' | 'agent_error' | 'bus_message' | 'model_route' | 'cron_trigger' | 'notification';
  agentId: string;
  action: string;
