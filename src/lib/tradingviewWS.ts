...
import { onTokenRefresh, refreshToken, getToken, initTokens } from '@/lib/tokenManager';
...
/** Get the active session ID (runtime override > tokenManager > env var) */
export function getSessionId(): string | null {
  return runtimeSessionId || getToken('tradingview', 'session_id') || process.env.TV_SESSION_ID || null;
}
...
/** Mark current session as invalid and attempt auto-refresh from Supabase */
export function markSessionInvalid(): void {
  sessionValid = false;
  sessionLastChecked = Date.now();
  // Auto-refresh: try to fetch a newer token from Supabase
  refreshToken('tradingview', 'session_id').then((newToken) => {
    if (newToken && newToken !== runtimeSessionId) {
      console.log('[TV WS] Auto-refreshed session from Supabase, reconnecting...');
...
// Register callback so when tokenManager refreshes the TV token, we reconnect
onTokenRefresh('tradingview', 'session_id', (newToken: string) => {
  if (newToken !== runtimeSessionId) {
    console.log('[TV WS] Token refreshed via tokenManager, reconnecting...');
    setSessionId(newToken);
  }
});
...
async function connectWebSocket(): Promise<void> {
  // Ensure tokens are loaded from Supabase on first connection
